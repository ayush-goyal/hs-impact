...
function imageError(error, b, c) {
	console.log('image error');
	if (error.code == 0) {
		$('.error-msg').text('Error loading image');
	} else if (error.code == 1) {
		$('.error-msg').text('Image size too small');
	}
...
$('.demo-wrapper.custom-button').cropit({
	onZoomEnabled: zoomEnabled,
	onZoomDisabled: zoomDisabled,
...
$('.select-image-btn').click(function() {
	...
	$('.cropit-preview').removeClass('has-error');
})

$('#rotate-right').click(function() {
	$('.demo-wrapper.custom-button').cropit('rotateCW');
});
$('#rotate-left').click(function() {
	$('.demo-wrapper.custom-button').cropit('rotateCCW');
});
...
$('.download-btn').click(function() {
	var imageData = $('.demo-wrapper.custom-button').cropit('export');
	if (!imageData) {
		alert('Please upload an acceptable file');
	}
	var blobData = dataURItoBlob(imageData);
	getSignedRequest(blobData);
});